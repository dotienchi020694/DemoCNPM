...
<body>
	...
	<ol class="breadcrumb">
	    ...
	    <li class="active">Quản lý quy đổi giờ chuẩn</li>
            <li class="active">Loại việc</li>
	</ol>

	<div class="panel panel-default panel-main-body">
		<div class="panel-heading">
			<h3 class="panel-title"><span class="glyphicon glyphicon-bomon" aria-hidden="true"></span>Danh sách loại việc</h3>
		</div>
		<div class="panel-body">
			<div class="bomon-function-form">
				<div class="form-left">
					<a href="?controller=loaiviec&action=add"><button type="button" class="btn btn-primary">Thêm mới</button></a>
				</div>
				<div class="form-right">
					<form class="input-group" method="POST" action="?controller=loaiviec&action=search" onsubmit="return checkBlankSearchInput()">
				...
				echo "<table class='table table-bordered'>";
					...
					foreach($result as $row) {
					 	echo "<tr>";
						 	...
						 	echo "<td>" . $row["lv_mota"] . "</td>";
						 	echo "<td style='text-align: center'>
						 		<a href='index.php?controller=loaiviec&action=edit&lv_id=".$row["lv_id"]."'><button class='btn btn-success'><span class='glyphicon glyphicon-edit' aria-hidden='true'></span>Sửa</button></a>
						 	</td>";
						 	echo "<td style='text-align: center'>
						 		<a href='index.php?controller=loaiviec&action=delete&lv_id=".$row["lv_id"]."'><button class='btn btn-danger'><span class='glyphicon glyphicon-trash' aria-hidden='true'></span>Xóa</button></a>
						 	</td>";
						echo "</tr>";
					}
				echo "</table>";
			    ...
			    <ul class="pagination">
			    	<?php
			    		...
			    		if (!isset($_GET["action"])) {
			    			echo "<a href='index.php?controller=loaiviec&action=search&page=".($_GET["page"]-1)."' aria-label='Previous'>";
			    		} else { 
				    		if ($_GET["action"] != "search") {
				    			echo "<a href='index.php?controller=loaiviec&page=".($_GET["page"]-1)."' aria-label='Previous'>";
				    		} else {
				    			echo "<a href='index.php?controller=loaiviec&action=search&page=".($_GET["page"]-1)."' aria-label='Previous'>";
				    		}
			    		}
			    		...
			    		for($i = 1; $i <= $page_number; $i++) {
			    			if(!isset($_GET["action"])) {
			    				echo "<li><a href='index.php?controller=loaiviec&page=".$i."'> ".$i."</a></li>";
			    			} else {
			    				if ($_GET["action"] != "search") {
				    				echo "<li><a href='index.php?controller=loaiviec&page=".$i."'> ".$i."</a></li>";
					    		} else {
					    			echo "<li><a href='index.php?controller=loaiviec&keyword=".$keyword."&action=search&page=".$i."'> ".$i."</a></li>";
					    		}
			    			}
			    		}
			    		echo "<li>";
			    		if(!isset($_GET["action"])) {
			    			echo "<a href='index.php?controller=loaiviec&action=search&page=".($_GET["page"]+1)."' aria-label='Next'>";
			    		} else { 
				    		if ($_GET["action"] != "search") {
				    			echo "<a href='index.php?controller=loaiviec&action=search&page=".($_GET["page"]+1)."' aria-label='Next'>";
				    		} else {
