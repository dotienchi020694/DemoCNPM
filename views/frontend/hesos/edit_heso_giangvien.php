			    ...
			    $("#checkNam").change(function(){
			    	if($(this).is(':checked')) {
						...
						$("#checkNamLonHon").attr('disabled', true);
			   			...
			   			$("#checkNamLonHon").attr('checked', false);
			   			$("#checkNamNhoHon").attr('checked', false);
		...
		<form class="panel panel-default panel-main-body" method="POST" action="">
			...
			<div class="panel-body">
				    ...
				    <div class="col-md-10">
				        ...
				        <?php
				        	foreach($hocvi as $row)  {
				        		echo "<option value=".$row["hv_id"]." ";
                                if($result["hv_id"]==$row["hv_id"]){
                                    echo "selected";}
                                echo ">" . $row["hv_ten"] . "</option>";
				        	}
				        ?>
				    	</select>
				    </div>
				    ...
				    <div class="col-md-10">
				        ...
				        <?php
				        	foreach($namhoc as $row)  {
				        		echo "<option value=".$row["nh_id"]." ";
                                if($result["nh_id"]==$row["nh_id"]){
                                     echo "selected";}
                                echo ">" . $row["nh_ten"] . "</option>";
				        	}
				        ?>
				    	</select>
				    </div>
				...
				<div class="form-group">
				    ...
				    <div class="col-md-10">
                        <input type="text" name="heso" class="form-control" id="inputEmail3" value=<?php echo $result["hs_heso"];?>>
				    </div>
				</div>
				<div class="form-group">
	                <label for="inputEmail3" class="col-sm-2 control-label">Số năm yêu cầu</label>
	                <div class="col-md-10">
	                    <?php echo '<input type="checkbox" aria-label="..." name="checkNam" value=1 id="checkNam"';
	                    if($result["hs_sonamyc"]!=0) { echo ' checked="checked"'; }
	                    echo '>'; ?>
	                </div>
                </div>
                <div class="require-year-form" style="padding: 10px; padding-left: 135px; padding-right: 25px; margin-bottom: -30px;">
                    <div class="input-group" style="margin-bottom: 10px;">
                        <span class="input-group-addon">
                            <?php echo '<input type="radio" name="sonam" value=2 id="checkNamLonHon" aria-label="..."';
                            if($result["hs_sonamdk"]==2) { echo ' checked="checked"'; }
                            if($result["hs_sonamyc"]==0) { echo ' disabled="disabled"'; }
                            echo '>'; ?>
                            </span>
                            <?php echo '<input type="text" name="checkNamLonHonValue" id="checkNamLonHonValue" class="form-control" aria-label="..." placeholder="Lớn hơn"';
                            if($result["hs_sonamdk"]==2) { echo ' value="'.$result["hs_sonamyc"].'"'; }
                            if($result["hs_sonamyc"]==0) { echo ' disabled="disabled"'; }
                            echo '>'; ?>
                        </div>
                        <div class="input-group">
                            <span class="input-group-addon">
                                <?php echo'<input type="radio" name="sonam" value=1 id="checkNamNhoHon" aria-label="..."';
                                if($result["hs_sonamdk"]==1) { echo ' checked="checked"'; }
                                if($result["hs_sonamyc"]==0) { echo ' disabled="disabled"'; }
                                echo'>'; ?>
                            </span>
                                <?php echo '<input type="text" name="checkNamNhoHonValue" id="checkNamNhoHonValue" class="form-control" aria-label="..." placeholder="Nhỏ hơn"';
                                if($result["hs_sonamdk"]==1) { echo ' value="'.$result["hs_sonamyc"].'"'; }
                                if($result["hs_sonamyc"]==0) { echo ' disabled="disabled"'; }
                                echo '>'; ?>
                        </div>
                </div>
				<div class="form-group">
				  	...
				  	<textarea class="form-control" name="mota" rows="5" id="comment"><?php echo $result["hs_mota"]?></textarea>
				</div>
