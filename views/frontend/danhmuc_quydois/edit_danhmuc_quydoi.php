		...
		<form class="panel panel-default panel-main-body" method="POST" action="">
			...
			<div class="panel-body">
				    ...
				    <div class="col-md-10">
				        ...
				        <?php
				        	foreach($loaiviec as $row)  {
				        		echo "<option value=".$row["lv_id"];
                                                        if($row["lv_id"]==$result["lv_id"])
                                                            echo "selected";
                                                        echo ">". $row["lv_ten"]."</option>" ;;
				        	}
				        ?>
				    	</select>
				    </div>
				...
				<div class="form-group">
				    ...
				    <div class="col-md-10">
                                        <input type="text" name="congviec" class="form-control" id="inputEmail3" placeholder="Tạo công việc cụ thể" value="<?php echo $result["dmqd_congvieccuthe"];?>">
				    </div>
				</div>
				    ...
				    <div class="col-md-10">
                                        <input type="text" name="gio" class="form-control" id="inputEmail3" placeholder="Nhập số giờ" value="<?php echo $result["dmqd_sogio"];?>">
				    </div>
				</div>
				    ...
				    <div class="col-md-10">
                                        <input type="text" name="sodonvi" class="form-control" id="inputEmail3" placeholder="Nhập số đơn vị" value="<?php echo $result["dmqd_sodonvi"];?>">
				    </div>
				</div>
				    ...
				    <div class="col-md-10">
				        ...
				        <?php
				        	foreach($donvi as $row)  {
				        		echo "<option value='".$row["dv_id"]."'";
                                                        if($row["dv_id"]==$result["dv_id"])
                                                        {
                                                            echo " selected";
                                                        }
                                                        echo ">". $row["dv_ten"]."</option>" ;;
				        	}
				        ?>
				    	</select>
				    </div>
				...
				<div class="form-group">
				  	...
				  	<textarea class="form-control" name="ghichu" rows="5" id="comment"><?php echo $result["dmqd_ghichu"]?></textarea>
				</div>
				<div class="button-form">
					<a href="index.php?controller=danhmuc_quydoi"><button type="button" class="btn btn-primary">Quay lại</button></a>
					<button id="update-btn" class="btn btn-success" type="submit" name="dmqd_id" value=<?php echo $_GET["dmqd_id"]; ?>>Cập nhật</button>
				</div>
			</div>
		</form>
	</body>
