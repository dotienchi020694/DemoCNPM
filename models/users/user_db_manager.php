<?php
	class User_db_manager {
		...
		function insert(User $new_user) {
			...
			$queryCommand = "INSERT INTO qltk_user(u_name, u_password, r_id, gv_id, u_trangthai) VALUES ('" . $new_u_name . "','" . $new_u_password . "', '" . $new_r_id . "', '" . $new_gv_id . "', '" . $new_u_trangthai . "')";
		...
		function edit(User $edit_user) {
			...
			$new_u_trangthai = $edit_user->get_u_trangthai();

			$queryCommand = "UPDATE qltk_user SET u_password ='" . $new_u_password . "', r_id = '" . $new_r_id . "', gv_id = '" . $new_gv_id . "', u_trangthai = '" . $new_u_trangthai . "' WHERE u_id = " . $u_id;
			$queryResult = $this->db_core->db_query($queryCommand);
		...
		function show_user_by_id($u_id) {
			...
			inner join qltk_role as r on u.r_id = r.r_id where u.is_delete=0 AND u.u_id = " . $u_id;
			$queryResult = $this->db_core->db_query($queryCommand);
		...
		function search_user_by_keyword($keyword) {
			$queryCommand = "select u.u_id, u.u_name, u.u_password, u.is_delete, g.gv_ten as giangvien_name, r.r_ten as role_name, u.u_trangthai 
			from qltk_user as u inner join qlgv_giangvien as g on u.gv_id = g.gv_id
			inner join qltk_role as r on u.r_id = r.r_id WHERE u.u_id LIKE '%" . $keyword . "%' OR u.u_password LIKE '%" . $keyword . "%' OR u.r_id LIKE '%" . $keyword . "%' OR u.gv_id LIKE '%" . $keyword . "%' OR u.is_delete LIKE '%" . $keyword . "%' OR g.gv_ten LIKE '%" . $keyword . "%'";
			$queryResult = $this->db_core->db_query($queryCommand);

			//echo $queryCommand; die();
		...
		function search_user_by_keyword_in_range($keyword, $first, $last) {
			$queryCommand = "select u.u_id, u.u_name, u.u_password, u.is_delete, g.gv_ten as giangvien_name, r.r_ten as role_name, u.u_trangthai 
			from qltk_user as u inner join qlgv_giangvien as g on u.gv_id = g.gv_id
			inner join qltk_role as r on u.r_id = r.r_id WHERE u.u_name LIKE '%" . $keyword . "%' OR u.u_password LIKE '%" . $keyword . "%' OR u.is_delete LIKE '%" . $keyword . "%' OR g.gv_ten LIKE '%" . $keyword . "%' LIMIT " . $first . ", " . $last;
		...
		function check_login_account($account) {
			...
			$queryCommand = "SELECT * FROM qltk_user WHERE u_name = '" . $username . "' AND u_password ='" . md5($password) . "'";
			$queryResult = $this->db_core->db_query($queryCommand);
			$result = $this->db_core->db_fetch_array($queryResult);
			if(!$result) { return NULL; }
			else { return $result; }
